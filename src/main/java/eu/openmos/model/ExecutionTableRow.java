...
/**
 * executionTable [tableId, Map( tableEntryId, productId, recipeId, recipeValidity, activityStatus, Frequency (? -> Task
 * 4.4)) )]
 *
...
public class ExecutionTableRow
{
  ...
  /**
   * WP3 semantic model alignment. Execution table single entry's unique ID.
   */
  private String uniqueId;
  /**
   * WP3 semantic model alignment. TODO check with Lboro if it is the product model id (or the product instance id).
   * 16/06/2017 - checked with lboro it could be either the product instance id or the product model id Product model
   * ID.
   */
  private String productId;
  /**
   * WP3 semantic model alignment. Recipe's ID.
   */
  private String recipeId;
  /**
   * The user could or could not decide which will be the next recipe. It depends on the skill reqs of the product: if
   * more than one skill req has the same "precedence" then the user can decide via HMI what should be the next recipe.
   * TBV with Pedro Ferreira Also some optimizer will be able to update this field and notify the MSB.
   */
  private String nextRecipeId;
  /**
   * WP3 semantic model alignment. Is the recipe valid or not.
   */
  // private boolean recipeValidity;
  /**
   * WP3 semantic model alignment. Is the recipe active or not.
   */
//    private boolean activityStatus;
  /**
   * WP3 semantic model alignment. Execution table single row frequency.
   *
   * MSB alignment. We don't need any frequency anymore.
   */
...
//    }
   ...
   *
   * unique id, product instance id or product model id, recipe id, recipe validity, activity status, frequency
   *
...
//        doc.append("frequency", frequency);
   ...
   *
   * uniqueId, productId, recipeId, recipeValidity, activityStatus, frequency
   *
