...
public class Recipe
{
   ...
   *
   * MSB alignment. on the msb side they have the adapter id, and on the adapter class they have the agent id
   */
  ...
  /**
   * WP3 semantic model alignment. TBV check with Lboro if list or single element 16/06/2017 -> one and only one skill,
   * confirmed by lboro, and it's required
   *
   * MSB alignment. they have a skill id
   */
  @XmlElement(name = "skill")
  private Skill skill;
  /**
   * WP3 semantic model alignment. Recipe's equipment.
   */
  private Equipment equipment;
  /**
   * WP3 semantic model alignment. Whether the recipe is optimized or not.
   */
  private boolean optimized = false;
  /**
   * WP3 semantic model alignment. Last optimization timestamp.
   */
  private Date lastOptimizationTimestamp;
  /**
   * WP3 semantic model alignment. Recipes's timestamp.
   */
  private Date registeredTimestamp;

  /**
   * Semantic model alignment v13
   */
   ...
   * @param kpisSetting - Recipe's KPI Settings. These must match the skill's KPIs.
   ...
   *
   * description, uniqueId, list of kpi settings, name, list of parameter settings, unique agent name, list of skill
   * requirements, list of skills, equipment, optimized, lastOptimizationTimestamp, msbProtocolEndpoint,
  ...
  public String toString()
  {
    ...
    builder.append(SerializationConstants.TOKEN_RECIPE);
    if (lastOptimizationTimestamp != null)
    {
      builder.append(lastOptimizationTimestamp.toString());
    } else
    {
      builder.append("null");
    }
          ...
          throws ParseException
  {
    ...
    if (!lastOptimizationTimestampString.equals("null"))
    {
      lastOptimizationTimestamp = sdf.parse(lastOptimizationTimestampString);
    }
   ...
   *
   * {
   * "description": description, "id": unique id, "kpiSettings": kpiSettings.toBSON(), "name": name,
   * "parameterSettings": parameterSettings.toBSON(), "uniqueAgentName": uniqueAgentName, "skillRequirements":
   * skillRequirements.toBSON(), "skillId": skillId, "equipmentId": equipmentId, "optimized": optimized,
   * "lastOptimizationTimestamp": lastOptimizationTimestamp, "registered": registeredTimestamp, }
   *
