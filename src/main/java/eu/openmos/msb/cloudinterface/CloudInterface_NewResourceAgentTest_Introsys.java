 ...
 */
public class CloudInterface_NewResourceAgentTest_Introsys {

    private static List<Recipe> myRecipes;
    private static List<Skill> mySkills;
    // PUT NEW INFO INTO RECIPES AND SKILLS AND SEND THEM. SOLVE PROBLEMS WITH THIS
...
//////////////////////////////
        ...
        CyberPhysicalAgentDescription cpad = getIntrosysTestObject(myAgentId);
        cpad.setType("resource_df_service");
    ...
    public static CyberPhysicalAgentDescription getIntrosysTestObject(String agentUniqueName) {
        CyberPhysicalAgentDescription cpad = new CyberPhysicalAgentDescription();
        cpad.setAgentClass("resource");
        cpad.setLogicalLocation(new LogicalLocation("lcgicallocation"));
        cpad.setPhysicalLocation(new PhysicalLocation("physicallocation", 1, 2, 3, 4, 5, 6));
        cpad.setType("resource_df_service");
        cpad.setUniqueName(agentUniqueName);
        cpad.setSkillRequirements(new LinkedList<>());
        cpad.setSkills(mySkills);
        cpad.setRecipes(myRecipes);
        // cpad.setAgentParameters(new LinkedList<>());
...
if(!file.exists()) { 
    throw new RuntimeException("skills.txt not found");
        ...
        try {
            ...
            while ((line = br.readLine()) != null) {
                mySkills.add(Skill.fromString(line));
            }
        } catch (FileNotFoundException ex) {
            System.out.println("Could not find file at introsysInfo/skills.txt.");
        } catch (IOException ex) {
            System.out.println("Could not open file at introsysInfo/skills.txt.");
        } finally {
            try {
                br.close();
            } catch (IOException ex) {
                System.out.println("Could not close file at introsysInfo/skills.txt.");
            }
        }
    }
    ...
    throw new RuntimeException("recipes.txt not found");
        ...
        try {
            ...
            String line;
            while ((line = br.readLine()) != null) {
                myRecipes.add(Recipe.fromString(line));
            }
        } catch (FileNotFoundException ex) {
            System.out.println("Could not find file at introsysInfo/recipes.txt.");
        } catch (IOException ex) {
            System.out.println("Could not open file at introsysInfo/recipes.txt.");
        } finally {
            try {
                br.close();
            } catch (IOException ex) {
                System.out.println("Could not close file at introsysInfo/recipes.txt.");
            }
        }
    }
