...
public class MyNodeManager extends NodeManagerUaNode {
	...
	private PlainMethod myMethod;
        ...
        private PlainMethod device_XML_Method;
        
        private PlainMethod general_method;

	private CallableListener myMethodManagerListener;
        
        private CallableListener device_XML_MethodManagerListener;
        
        private CallableListener general_method_MethodListener;
	...
	final MyEventManagerListener myEventManagerListener = new MyEventManagerListener();
        private CacheVariable myJustadd;
         
       private final HashMap myNodesMap = new HashMap<String, UaVariableNode>();
	...
	private void createAddressSpace() throws StatusException, UaInstantiationException {
		...
		final NodeId myDeviceId = new NodeId(ns, "DeviceTopics");
		myDevice = new UaObjectNode(this, myDeviceId, "DeviceTopics", Locale.ENGLISH);
		...
		myDevice.addComponent(myLevel);
                ...
                final NodeId JustAddedTypeId = new NodeId(ns, "JustAddedType");
		UaType JustAddedType = this.addType(JustAddedTypeId, "JustAddedType", baseDataVariableType);
                
                final NodeId myJustAddedNodeID = new NodeId(ns, "IJustAddedANode");
		UaType tripleType = getServer().getNodeManagerRoot().getType(Identifiers.Double);
		myJustadd = new CacheVariable(this, myJustAddedNodeID, "IJustAddedANode", LocalizedText.NO_LOCALE);
		myJustadd.setDataType(tripleType);
		myJustadd.setTypeDefinition(JustAddedType);
                myJustadd.setDescription(LocalizedText.english("I am a cool node!"));
		myDevice.addComponent(myJustadd);
                ((CacheVariable) myJustadd).updateValue((int) 999666);
                /*end custom*/
	...
	private void createMethodNode() throws StatusException {
		int ns = this.getNamespaceIndex();
		final NodeId myMethodId = new NodeId(ns, "MyTrigonometryMethod");
                final NodeId DeviceXMLmethodId = new NodeId(ns, "DeviceXMLmethod");
                final NodeId GeneralMethodId = new NodeId(ns, "GeneralMethod");
                
		myMethod = new PlainMethod(this, myMethodId, "MyTrigonometryMethod", Locale.ENGLISH);
                device_XML_Method = new PlainMethod(this, DeviceXMLmethodId, "DeviceXMLmethod", Locale.ENGLISH);
                general_method = new PlainMethod(this, GeneralMethodId, "GeneralMethod", Locale.ENGLISH);
		...
		myMethod.setOutputArguments(outputs);
                ...
                //device_XML_Method arguments
                Argument[] inputsXML = new Argument[1];
		inputsXML[0] = new Argument();
		inputsXML[0].setName("XML_info");
		inputsXML[0].setDataType(Identifiers.String);
		inputsXML[0].setValueRank(ValueRanks.Scalar);
		inputsXML[0].setArrayDimensions(null);
		inputsXML[0].setDescription(new LocalizedText("input a XML containing device info to MSB", Locale.ENGLISH));
                device_XML_Method.setInputArguments(inputsXML);
                
                Argument[] outputsXML = new Argument[1];
		outputsXML[0] = new Argument();
		outputsXML[0].setName("Feedback");
		outputsXML[0].setDataType(Identifiers.String);
		outputsXML[0].setValueRank(ValueRanks.Scalar);
		outputsXML[0].setArrayDimensions(null);
		outputsXML[0].setDescription(new LocalizedText("MSB feedback message", Locale.ENGLISH));
		device_XML_Method.setOutputArguments(outputsXML);
                
                //general_method arguments
                Argument[] gen_inputs = new Argument[2];
		gen_inputs[0] = new Argument();
		gen_inputs[0].setName("Operation");
		gen_inputs[0].setDataType(Identifiers.String);
		gen_inputs[0].setValueRank(ValueRanks.Scalar);
		gen_inputs[0].setArrayDimensions(null);
		gen_inputs[0].setDescription(new LocalizedText(
				"The operation to perform on parameter: valid functions are sin, cos, tan, pow", Locale.ENGLISH));
		gen_inputs[1] = new Argument();
		gen_inputs[1].setName("Parameter");
		gen_inputs[1].setDataType(Identifiers.String);
		gen_inputs[1].setValueRank(ValueRanks.Scalar);
		gen_inputs[1].setArrayDimensions(null);
		gen_inputs[1].setDescription(new LocalizedText("The parameter for operation", Locale.ENGLISH));
		general_method.setInputArguments(gen_inputs);

		Argument[] gen_outputs = new Argument[1];
		gen_outputs[0] = new Argument();
		gen_outputs[0].setName("Result");
		gen_outputs[0].setDataType(Identifiers.String);
		gen_outputs[0].setValueRank(ValueRanks.Scalar);
		gen_outputs[0].setArrayDimensions(null);
		gen_outputs[0].setDescription(new LocalizedText("The result of 'operation(parameter)'", Locale.ENGLISH));
		general_method.setOutputArguments(gen_outputs);
                
                //********************
                
		this.addNodeAndReference(myDevice, myMethod, Identifiers.HasComponent);
                this.addNodeAndReference(myDevice, device_XML_Method, Identifiers.HasComponent);
                this.addNodeAndReference(myDevice, general_method, Identifiers.HasComponent);

		// Create the listener that handles the method calls
		myMethodManagerListener = new MyTrigonometryMethodManagerListener(myMethod);
                device_XML_MethodManagerListener = new deviceInfoXMLmethodManagerListener(device_XML_Method);
                general_method_MethodListener = new GeneralMethodManagerListener(general_method);
		
                MethodManagerUaNode m = (MethodManagerUaNode) this.getMethodManager();
		m.addCallListener(myMethodManagerListener);
                m.addCallListener(device_XML_MethodManagerListener);
                m.addCallListener(general_method_MethodListener);

                //MethodManagerUaNode n = (MethodManagerUaNode) this.getMethodManager();
                //n.addCallListener(device_XML_MethodManagerListener);
	}
	...
	void addNode(String node_name) throws StatusException {
                int ns = getNamespaceIndex();
		// My Event Manager Listener
		this.getEventManager().setListener(myEventManagerListener);
		// UA types and folders which we will use
		...
		final UaType baseDataVariableType = getServer().getNodeManagerRoot().getType(Identifiers.BaseDataVariableType);
                ...
                final NodeId JustAddedTypeId = new NodeId(ns, node_name + "id");
		UaType JustAddedType = this.addType(JustAddedTypeId, node_name + "id", baseDataVariableType);
                
                final NodeId myJustAddedNodeID = new NodeId(ns, node_name);
		UaType doubleType = getServer().getNodeManagerRoot().getType(Identifiers.Double);
		myJustadd = new CacheVariable(this, myJustAddedNodeID, node_name, LocalizedText.NO_LOCALE);
		myJustadd.setDataType(doubleType);
		myJustadd.setTypeDefinition(JustAddedType);
		myDevice.addComponent(myJustadd);
                
                myNodesMap.put(node_name, myJustadd);
                //((CacheVariable) myJustadd).updateValue((int) 999666);
        ...
        /*custom method @rmatos*/
        void changeNodeValue(String node_name, Object value) throws StatusException{
           ((CacheVariable)myNodesMap.get(node_name)).updateValue(value);
	...
	void deleteNode(/*QualifiedName*/String  nodeName) throws StatusException {
                QualifiedName node_name = new QualifiedName(this.getNamespaceIndex(), nodeName);
            
		UaNode node = myObjectsFolder.getComponent(/*nodeName*/node_name);
		if (node != null) {
			getServer().getNodeManagerRoot().beginModelChange();
			try {
