...
/**
 * Object that represents the data that resource and transport agents receive from the Manufacturing Service Bus
 * (related to workstations, transport stations) and that then are passed on to the optimizer.
 *
...
public class RawEquipmentData
{
  ...
  /**
   * WP3 semantic model alignment It matches with (it replaces) the former field "cloud agent name"
   */
  ...
  /**
   * WP3 semantic model alignment. Timestamp of sensors data.
   */
   ...
   *
   * equipmentId, array of sensor values, value type TO CHECK WITH PEDRO LBORO, measurement unit TO CHECK WITH PEDRO
   * LBORO, registeredTimestamp ("yyyy-MM-dd HH:mm:ss.SSS")
   *
