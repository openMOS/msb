...
public class Constants
{
  ...
  /**
   * Name of the property into the default.properties file that define the host where the agent cloud jade platform is
   * running. Example: openmos.agentplatform.host=localhost
   */
  public static final String PLATFORM_HOST_PARAMETER = "openmos.agentplatform.host";

  /**
   * Name of the property into the default.properties file that define the port number where the agent cloud jade
   * platform is listening. Example: openmos.agentplatform.port=1099
   */
  ...
  /**
   * Name of the property into the default.properties file that define if the Jade graphical console has to be started
   * or not. Default value is false. Example: openmos.agentplatform.graphical.console=true
   */
  public static final String PLATFORM_GUI_PARAMETER = "openmos.agentplatform.graphical.console";

  /**
   * Name of the property into the default.properties file that define the port number where the agent cloud bootstrap
   * class is listening for shutdown messages. Example: openmos.agentplatform.shutdown.port=9998
   */
  public static final String PLATFORM_SHUTDOWN_PORT_PARAMETER = "openmos.agentplatform.shutdown.port";

  /**
   * Name of the property into the default.properties file that locates the MSB service for recipes deployment. Example:
  ...
  /**
   * Name of the property into the default.properties file that locates the agent data web-service, e.g. the service
   * that wraps the agent-cloud database repository. The agent data web-service is currently not in use. Example:
  ...
  /**
   * Name of the property into the default.properties file that locates the data cloud web-service, e.g. the service
   * that wraps the tracking repository located somewhere in the cloud. The data cloud web-service is currently not in
   * use. Example: openmos.data.cloud.ws.endpoint=http://localhost:8888/wsDataCloudMessageTracker
   */
   ...
   * The class must be located into the agent-cloud java classpath at the platform startup time. Example:
  ...
  /**
   * Name of the property into the default.properties file that locates the cloud interface web-service, e.g. the
   * entry-point web-service for the platform. This web-service exposes for example calls for agent creation and
   * removal. Example: openmos.agent.systemconfigurator.ws.endpoint=http://localhost:9999/wsSystemConfigurator
   */
  public static final String AGENT_SYSTEMCONFIGURATOR_WS_PARAMETER = "openmos.agent.systemconfigurator.ws.endpoint";

  /**
   * Name of the property into the default.properties file that locates the shutdown web-service, e.g. the service
   * called by the bootstrap class when the user request the shutdown of the entire platform. The service takes care of
   * a clean agents removal before the platform is shutted down. Example:
  ...
  /**
   * It defines messages of type "life beat" for both resource and transport agents, coming from the Manifacturing
   * Service Bus to the agent cloud via p2p communication (websocket).
   */
  public static String MSB_MESSAGE_TYPE_LIFEBEAT = "_MESSAGE_LIFEBEAT_";

  /**
   * It defines messages of type "extracted data" for both resource and transport agents, coming from the Manifacturing
   * Service Bus to the agent cloud via p2p communication (websocket).
   */
  public static String MSB_MESSAGE_TYPE_EXTRACTEDDATA = "_MESSAGE_EXTRACTEDDATA_";

  /**
   * It defines messages of type "new location" for transport agents only, coming from the Manifacturing Service Bus to
   * the agent cloud via p2p communication (websocket).
   */
  public static String MSB_MESSAGE_TYPE_NEWLOCATION = "_MESSAGE_NEWLOCATION_";

  /**
   * It defines messages of type "applied recipes" for resource agents only, coming from the Manifacturing Service Bus
   * to the agent cloud via p2p communication (websocket).
   */
  public static String MSB_MESSAGE_TYPE_APPLIEDRECIPES = "_MESSAGE_APPLIEDRECIPES_";

  /**
   * It defines messages of type "suicide" for both resource and transport agents, coming from the Manifacturing Service
   * Bus to the agent cloud via p2p communication (websocket).
   */
  public static String MSB_MESSAGE_TYPE_SUICIDE = "_SUICIDE_";

  /**
   * It defines a category for messages of an unknown type, coming from the Manifacturing Service Bus to the agent cloud
   * via p2p communication (websocket).
   */
